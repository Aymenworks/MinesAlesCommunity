...
- (id)initWithFrame:(CGRect)frame {
    if(self = [super initWithFrame:frame]) {
        [self setBackgroundColor:datePickerMainColor];
...
- (void)drawRect:(CGRect)rect {
    ...
    CGContextSetStrokeColorWithColor(context, (self.state != UIControlStateHighlighted) ? datePickerMainColor.CGColor : SELECTED_TEXT_COLOR.CGColor);
...
- (id)initWithFrame:(CGRect)frame andValues:(NSArray *)arrayValues
    ...
    if(self = [super initWithFrame:frame]) {
        ...
        _cellFont = [UIFont systemFontOfSize:txtSize];
        _boldCellFont = [UIFont boldSystemFontOfSize:txtSize];
...
@property (nonatomic, strong) SBPickerButton *cancelButton;
...
- (void)buildControl {
    ...
    [saveArea setBackgroundColor:datePickerBackgroundColor];
  
    //Create save button
    CGFloat buttonWidth = self.frame.size.width/2-20.0;
    _cancelButton = [[SBPickerButton alloc] initWithFrame:CGRectMake(13.0, 10.0, buttonWidth, SAVE_AREA_HEIGHT-20.0)];
    _saveButton = [[SBPickerButton alloc] initWithFrame:CGRectMake(27.0 + buttonWidth , 10.0, buttonWidth, SAVE_AREA_HEIGHT-20.0)];
  
  [_cancelButton setTitle:NSLocalizedString(@"Cancel", comment: @"Button title that cancel the debt reminder picker controller").uppercaseString  forState:UIControlStateNormal];
...
- (NSDate *)createDateWithFormat:(NSString *)format andDateString:(NSString *)dateString {
    ...
    NSString *fullDateString = [NSString stringWithFormat:dateString,
    ...
    return [formatter dateFromString:fullDateString];
} //edit here
